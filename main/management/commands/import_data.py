...
class Command(BaseCommand):
    help = "Import products in Freshmarket"
    ...
    def handle(self, *args, **options):
        ...
        for row in reader:
            cate = row["category"]
            print(cate)
            category1 = models.Category.objects.get(slug=cate)
            print(category1)
            
            product, created = models.Product.objects.get_or_create(
                name=row["name"], old_price=row["old_price"], new_price=row["new_price"],
                category =category1 
            )
        ...
        self.stdout.write(
            "Category processed=%d (created=%d)" % (c["category"], c["category_created"])
        )
